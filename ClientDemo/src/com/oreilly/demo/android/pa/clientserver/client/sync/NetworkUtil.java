...
public final class NetworkUtil {
	...
	public static JSONObject login(String baseurl, String username,
			String password, boolean oldtoken, Handler handler) {
	...
	public static JSONObject getFriends(String baseurl, String token,
			Handler handler) {
	...
	public static JSONObject getFriends(String baseurl, String token,
			long time, Handler handler) {

		if (handler != null)
			getJSONAsync(baseurl + GETFRIENDS + TOKEN + token + "&" + TIME
					+ time, handler);
		else {
			try {
				return getJSON(baseurl + GETFRIENDS + TOKEN + token + "&"
						+ TIME + time);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static void addFriendUser(String baseurl, String token,
			String userstr, Handler handler) {

		addFriend(baseurl, token, null, userstr, handler);

	}

	public static void addFriend(String baseurl, String token, String friendid,
			Handler handler) {
	...
	public static void addFriend(String baseurl, String token, String friendid,
		...
		getJSONAsync(baseurl
				+ ADDFRIEND
				+ TOKEN
				+ token
				+ (friendid != null ? "&" + ID + friendid
						: (userstr != null ? "&" + USER + userstr : "")),
				handler);

	}

	public static void deleteFriend(String baseurl, String token,
			String friendid, Handler handler) {

		getJSONAsync(baseurl + DELETEFRIEND + TOKEN + token
				+ (friendid != null ? "&" + ID + friendid : ""), handler);
	}

	// -- END API CALLS TO SERVER -- //

	/**
	 * async http method that gets JSONObject response and passes it to the
	 * handler
	 * 
	 * @param url
		...
		pool.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (handler != null)
						Message.obtain(handler, ERR, t).sendToTarget();
					else
						t.printStackTrace();
