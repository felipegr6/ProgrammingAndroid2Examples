...
public class AccountAddActivity extends Activity {
	...
	private void add() {
		...
		if (name.getText() == null
			...
			Toast.makeText(this, "Contacts must have a name",
					Toast.LENGTH_SHORT);
		...
		// we are not assigning to any particular account but if we wish to we
		// would
		// get the accounts this way
		/*
		 * Account[] accounts = AccountManager.get(this).getAccounts();
		 * accountname = accounts[0].name; accounttype = accounts[0].type;
		 */
		...
		ops.add(ContentProviderOperation
				...
				.withValue(
						...
						name.getText().toString().trim()).build());
		...
		try {
			...
			e.printStackTrace();
			Toast.makeText(this, "Something bad happened! " + e.getMessage(),
					Toast.LENGTH_SHORT);
