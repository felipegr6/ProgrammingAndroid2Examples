...
public class SyncAdapter extends AbstractThreadedSyncAdapter {
	...
	public void onPerformSync(Account account, Bundle extras, String authority,
		...
		try {
			...
			authtoken = AccountManager.get(context).blockingGetAuthToken(
					account, Authenticator.AUTHTOKEN_TYPE, true);

			ListFriends friendsdata = ListFriends.fromJSON(NetworkUtil
					.getFriends(
							NetworkUtil.hosturl != null ? NetworkUtil.hosturl
	...
	private ArrayList<ContentProviderOperation> add(Account account, User f) {
		...
		ops.add(ContentProviderOperation
				...
				.withValue(ContactsContract.Data.DATA3, "View profile").build());
	...
	private long lookupRawContact(long id) {
		...
		Cursor c = context.getContentResolver().query(
				...
				RawContacts.ACCOUNT_TYPE + "='" + Authenticator.ACCOUNT_TYPE
						+ "' AND " + RawContacts.SYNC1 + "=?",
